...
public final class ParseUtil {
    ...
    public static long parseHertz(String hertz) {
        ...
        if (matcher.find() && matcher.groupCount() == 3) {
            ...
            double value = Double.valueOf(matcher.group(1)) * multipliers.get(matcher.group(3));
            if (value >= 0d) {
                return (long) value;
            }
        }
    ...
    public static List<String> parseByteArrayToStrings(byte[] bytes) {
        ...
        do {
            ...
            if (end == bytes.length || bytes[end] == 0) {
                ...
                strList.add(new String(bytes, start, end - start, UTF_8));
                start = end + 1;
            }
    ...
    public static Map<String, String> parseByteArrayToStringMap(byte[] bytes) {
        ...
        do {
            ...
            if (end == bytes.length || bytes[end] == 0) {
                ...
                // Intentionally using platform default charset
                strMap.put(key, new String(bytes, start, end - start, UTF_8));
                key = null;
                start = end + 1;
            } else if (bytes[end] == '=' && key == null) {
                key = new String(bytes, start, end - start, UTF_8);
                start = end + 1;
            }
        } while (end++ < bytes.length);
        return strMap;
